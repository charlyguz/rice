...
-- Android emulator: keep it off whatever workspace is focused (it otherwise
-- pops up over fullscreen games). Show it with:
--   hyprctl dispatch togglespecialworkspace android
local androidEmulator = "^([Ee]mulator|qemu-system.*|.*[Aa]ndroid.?[Ee]mulator.*)$"
local androidWorkspace = "special:android silent"

hl.window_rule({ match = { class = androidEmulator }, workspace = androidWorkspace })
hl.window_rule({ match = { title = "^(Android Emulator.*|Emulator\\s-\\s.*|.*:5554.*)$" }, workspace = androidWorkspace })
